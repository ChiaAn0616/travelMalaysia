...
function updatePrices(currency) {
    ...
    events.forEach(event => {
        ...
        if (basePrice !== 0) {
            ...
            priceElement.textContent = "From: " + displayPrice;
        } else {
...
function filterEvents() {
    ...
    const selectedFacilities = document.getElementById('facilitiesFilter').value.toLowerCase();
    const currency = getCurrencyPreference() || 'MYR'; // Get user preferred currency
    const events = document.querySelectorAll('.event-card');
    ...
    events.forEach(function(event) {
        ...
        const facilities = event.dataset.facilities.toLowerCase().split(',').map(c => c.trim());
        const basePrice = parseFloat(event.dataset.price);
        const eventStartDate = new Date(event.dataset.startdate);
        const eventEndDate = new Date(event.dataset.enddate);
        const stars = parseInt(event.dataset.stars);
        const convertedPrice = convertPrice(basePrice, currency);

        // Filter checks
        const matchesQuery = title.includes(query) || venue.includes(query);
        const matchesState = selectedState === '' || state === selectedState;
        const matchesStars = selectedStars == '' || stars == selectedStars;
        const matchesCategory = selectedCategory === '' || categories.includes(selectedCategory);
        console.log(categories);
        console.log(selectedCategory);

        const matchesFacilities = selectedFacilities === '' || facilities.includes(selectedFacilities);
        ...
        const showEvent = matchesQuery && matchesState && matchesCategory && matchesDate && matchesPrice && matchesStars && matchesFacilities;
        if (showEvent) {
...
function clearFilter() {
    ...
    document.getElementById('startDateFilter').value = '';     // Clear date inputs
    document.getElementById('endDateFilter').value = '';
    document.getElementById('starsFilter').value = '';
    document.getElementById('facilitiesFilter').value = '';
...
document.getElementById('starsFilter').addEventListener('change', filterEvents);
document.getElementById('facilitiesFilter').addEventListener('change', filterEvents);
